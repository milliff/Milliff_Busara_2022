...
author = 'Lingbo Huang, edied by Aidan Milliff'
...
class Constants(BaseConstants):
    name_in_url = 'ambiguity_game'
    players_per_group = None
    num_rounds = 1
    # DIFFICULTYCHOICES = ["easy", "hard"]
...
class Subsession(BaseSubsession):
    ...
    def creating_session(self):
        for p in self.get_players():
            p.prize = 5
            p.treatment = random.choice(["easy", "hard"])
...
class Player(BasePlayer):
    ...
    def set_payoff(self):
        if (self.catches  >= 10):
            ...
            self.payoff = 50 - (self.catches * self.prize)
